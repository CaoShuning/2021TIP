function [B,k] = AddBlur(img,SIZ,SIG,pad)
% 
% [M, N] = size(img);
k = im2double(fspecial('gaussian',SIZ,SIG));
% I_pad = wrap_boundary_liu(img,[M N]+size(k)-1);
% 
% I_pad = imrotate(I_pad, 180);
% % % figure,imshow(I_pad);
% [M, N] = size(I_pad);
% I_pad = wrap_boundary_liu(I_pad,[M N]+size(k)-1);
% I_pad = imrotate(I_pad, 180);
% % figure,imshow(I_pad);
...
% figure,imshow(f);
% [M, N] = size(I_pad);
% B = f(size(k):M-size(k)+1,size(k):N-size(k)+1);
k = zpad(k,pad, pad);
% B = f(1:M,1:N);
% figure,imshow(B);
% imshow(blur_img);

...
addpath(genpath('D:\PAPER\paper\2016 CVPR Blind Image Deblurring Using Dark Channel Prior\codes\'));
...
[Gname, Path_g] = uigetfile(Path_g,'*.tif');
g = im2double(imread([Path_g, Gname]));

Method = 'WFAF'; date = '19-Jan-2021';
Path_F = ['C:\Users\caoshuning\Desktop\Submit_text\result\simulation\',Method,'\',date,'\F\'];
[Fname, Path_F] = uigetfile([Path_F,'*.tif']);
y = im2double(imread([Path_F, Fname]));

Path_Ori = 'D:\caoshuning\code_DestripeDeblur\Data\ori\';
[Oriname, Path_Ori] = uigetfile([Path_Ori,'*.tif']);
ori = im2double(imread([Path_Ori, Oriname]));
...
opts.kernel_size = 9;  saturation = 1;
lambda_dark = 4e-3; lambda_grad = 4e-3; 
